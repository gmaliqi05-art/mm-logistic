-- ============================================================================
-- PENDING MIGRATION: Delivery Flow Roles, Counterparty Detection, Custody Stock
-- ----------------------------------------------------------------------------
-- Apply this via the Supabase dashboard SQL editor (or the mcp__supabase
-- migration tool when available). The client code already reads/writes these
-- columns; run this migration to activate the backend logic.
--
-- SUMMARY
--   delivery_notes: adds flow_role + counterparty_* + owner_company_id +
--     origin_depot_id/destination_depot_id + touches_own_stock.
--   stock / stock_movements: adds ownership ('own' | 'held') + owner_company_id,
--     plus stock_movements.delivery_note_id.
--   depots: lat, lng, depot_type.
--   partner_flow_events (new table): cross-company flow audit; RLS so both the
--     owning company and the partner company can see the rows.
--   match_counterparty_company(vat,email,phone,name) helper function.
--   emit_partner_flow_events() trigger fires on delivery_notes status
--     transitions into delivered/confirmed and populates partner_flow_events.
--
-- SECURITY
--   partner_flow_events has RLS with separate select/insert/update/delete
--   policies, restricted to the owning company and (for select only) the
--   matched partner company.
-- ============================================================================
