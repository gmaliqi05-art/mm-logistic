/*
  # L2: NULL-safe stock matching in process_delivery_note_stock

  process_delivery_note_stock used `=` to match the stock row to
  deduct from. SQL evaluates `NULL = NULL` to NULL (falsy), so
  delivery_note_items with category_product_id IS NULL (the
  sorting-bound or repair-bound intakes) silently failed to
  decrement the bare-category damaged stock row on the consignor
  and internal_transfer paths.

  Fix: replace `=` with `IS NOT DISTINCT FROM` everywhere we match
  stock by category_product_id. No data backfill — past delivery
  notes that hit the bug already left drift; reconciling that is a
  separate manual task because the stock_movements rows exist and
  the deltas can be computed but the right physical state must be
  ...
  LOOP
    IF v_role = 'consignor' THEN
      ...
      WHERE company_id = NEW.company_id
        ...
        AND category_product_id IS NOT DISTINCT FROM item.category_product_id
        ...
        BEGIN
          EXECUTE format(
            'UPDATE held_stock SET quantity = GREATEST(0, quantity - $1)
             WHERE company_id = $2 AND depot_id = $3
               AND category_product_id IS NOT DISTINCT FROM $4
