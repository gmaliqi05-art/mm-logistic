/*
  # Unique constraint on sorting batch source_item + rewrite trigger for idempotency

  1. Changes
    - Add UNIQUE partial index on `pallet_sorting_batches(source_item_id)` where source_item_id IS NOT NULL
      to prevent duplicate batches for the same delivery note item
    - Rewrite the `sorting` section of `process_delivery_note_stock()` trigger to use
      INSERT ... ON CONFLICT instead of SELECT + conditional INSERT, making it truly idempotent
    - This fixes the `pallet_sorting_batches_status_check` constraint violation that occurs
      when the company creates batches via handleSendToSorting and the trigger tries to create
      duplicates when the note status changes to confirmed

  2. Security
    - No RLS changes
    - Trigger remains SECURITY DEFINER with explicit search_path
*/

-- 1. Add unique partial index on source_item_id
CREATE UNIQUE INDEX IF NOT EXISTS uq_sorting_batch_source_item
  ON pallet_sorting_batches(source_item_id)
  WHERE source_item_id IS NOT NULL;

-- 2. Rewrite the trigger function with ON CONFLICT for sorting batches
...
BEGIN
  ...
  LOOP
    ...
    ELSIF route = 'sorting' THEN
      -- Use ON CONFLICT with the unique partial index for true idempotency.
      -- If the batch already exists (created by the frontend), just update it.
      ...
      -- Only notify depot workers if this is a new batch (not an update)
