/*
  # Backfill repair stock to specific products (v4)

  Loosens the category-name matching so "europalette" (legacy) maps to
  "Euro Paletten" (current). v4 also matches when one name contains the other
  after spaces are stripped. Idempotent.
*/
...
BEGIN
  CREATE TEMP TABLE IF NOT EXISTS tmp_repair_alloc_v4 (
    company_id uuid,
    depot_id uuid,
    category_id uuid,
  ...
  resolved_cat AS (
    ...
    FROM classified c
    LEFT JOIN LATERAL (
      SELECT pc_inner.id FROM product_categories pc_inner
      WHERE pc_inner.company_id = c.company_id
        AND (
          ...
          OR replace(lower(trim(pc_inner.name)), ' ', '') LIKE replace(c.cname, ' ', '') || '%'
          OR replace(c.cname, ' ', '') LIKE replace(lower(trim(pc_inner.name)), ' ', '') || '%'
        )
      ORDER BY
        CASE WHEN lower(trim(pc_inner.name)) = c.cname THEN 0
             WHEN replace(lower(trim(pc_inner.name)), ' ', '') = replace(c.cname, ' ', '') THEN 1
             ELSE 2 END
      LIMIT 1
    ) pc ON true
    WHERE pc.id IS NOT NULL
  )
  INSERT INTO tmp_repair_alloc_v4 (company_id, depot_id, category_id, category_product_id, quantity)
  SELECT rc.company_id, rc.depot_id, rc.resolved_category_id, cp.id, SUM(rc.qty)
  FROM resolved_cat rc
  LEFT JOIN LATERAL (
    SELECT cp_inner.id FROM category_products cp_inner
    WHERE cp_inner.company_id = rc.company_id
      AND cp_inner.category_id = rc.resolved_category_id
      AND (
        lower(trim(cp_inner.name)) = rc.pname
        OR replace(lower(trim(cp_inner.name)), ' ', '') = replace(rc.pname, ' ', '')
        OR (
    ...
    ORDER BY CASE WHEN lower(trim(cp_inner.name)) = rc.pname THEN 0 ELSE 1 END, cp_inner.name LIMIT 1
  ) cp ON true
  ...
  LOOP
    ...
    FROM tmp_repair_alloc_v4
    WHERE company_id = v_stock.company_id AND depot_id = v_stock.depot_id AND category_id = v_stock.category_id;

    IF v_total_repaired <= 0 THEN CONTINUE; END IF;
    v_remaining := v_stock.quantity;

    FOR v_alloc IN
      SELECT category_product_id, quantity FROM tmp_repair_alloc_v4
      WHERE company_id = v_stock.company_id AND depot_id = v_stock.depot_id AND category_id = v_stock.category_id
    ...
    LOOP
      ...
      IF v_allocated > v_remaining THEN v_allocated := v_remaining; END IF;

      SELECT id INTO v_existing_id FROM stock
      WHERE company_id = v_stock.company_id AND depot_id = v_stock.depot_id AND category_id = v_stock.category_id
        AND category_product_id = v_alloc.category_product_id AND condition = 'good' LIMIT 1;
