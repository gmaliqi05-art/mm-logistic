/*
  # A-M1 (targeted): carrier deliveries must not touch our own stock

  `process_delivery_note_stock` routes purely off `NEW.type`
  (delivery → exit, else → entry) and `assigned_depot_id`; it never consults
  `our_role`. So a delivery note where our company is only the **carrier**
  (we transport goods that belong to the consignor / consignee, they are
  never our inventory) still books an `exit`/`entry` against our stock — a
  cross-owner stock corruption.

  This adds a single early guard: when `our_role = 'carrier'` the trigger
  skips stock posting entirely (no depot required, no movement booked) and
  marks the note as processed so it isn't retried. Everything else is
  reproduced verbatim from the live function.

  Scope is deliberately limited to `carrier` (verified: zero carrier rows
  exist today, so this is forward-only and cannot corrupt current data).
  Custody (`custodian_in`/`custodian_out`) and the full unification of the
  overlapping `type` / `flow_role` / `our_role` vocabularies (A-M1/A-M2)
  remain a separate, deliberate refactor tracked in
  docs/DEEP_AUDIT_AND_STRATEGY.md.
*/
...
BEGIN
  ...
  -- Carrier: we only transport these goods (they belong to the consignor /
  -- consignee), so they never enter our own inventory. Skip stock posting
  -- entirely — no depot is required and no movement is booked.
