    ...
    <div className="p-4 lg:p-6 max-w-5xl mx-auto space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          <Link to="/company/pallet-accounts" className="p-1.5 rounded-lg hover:bg-slate-100 flex-shrink-0">
            <ArrowLeft className="w-4 h-4 text-slate-600" />
          </Link>
          <div className="min-w-0">
            <h1 className="text-base lg:text-lg font-semibold text-slate-900 truncate">{header.partner_name}</h1>
            <p className="text-xs text-slate-500">{header.pallet_type} pallet ledger</p>
          </div>
        </div>
        <div className="flex gap-2 flex-shrink-0">
          <button onClick={() => setShowAdj(true)} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-slate-300 text-xs font-medium text-slate-700 hover:bg-slate-50">
            <Plus className="w-3.5 h-3.5" /> Adjustment
          </button>
          <button onClick={generatePdf} disabled={pdfBusy} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-teal-600 text-white text-xs font-semibold hover:bg-teal-700 disabled:opacity-50">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        <div className="bg-white rounded-lg border border-slate-200 p-3">
          <div className="text-[10px] text-slate-500 uppercase tracking-wide">Current balance</div>
          <div className={`text-lg lg:text-xl font-bold mt-0.5 ${header.current_balance > 0 ? 'text-emerald-700' : header.current_balance < 0 ? 'text-red-700' : 'text-slate-900'}`}>
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-3">
          <div className="text-[10px] text-slate-500 uppercase tracking-wide">Opening balance</div>
          <div className="text-lg lg:text-xl font-bold text-slate-900 mt-0.5">{header.opening_balance}</div>
        </div>
        <div className="bg-white rounded-lg border border-slate-200 p-3">
          <div className="text-[10px] text-slate-500 uppercase tracking-wide">{t('common.totalTransactions')}</div>
          <div className="text-lg lg:text-xl font-bold text-slate-900 mt-0.5">{txns.length}</div>
        </div>
      </div>
