...
import { ArrowDownLeft, ArrowUpRight, Handshake, Warehouse, Package, Loader2 } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error } = await supabase
        ...
        .select(
          '*, partner_company:companies!partner_flow_events_partner_company_id_fkey(id,name), partner_contact:acc_contacts(id,name), category:product_categories(id,name), delivery_note:delivery_notes(note_number)',
        )
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((r) => {
                ...
                const partnerName =
                  r.partner_company?.name ?? r.partner_contact?.name ?? '—';
                ...
                return (
                  <tr key={r.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-2 text-slate-600">{r.delivery_note?.note_number ?? '—'}</td>
                    ...
                    <td className="px-4 py-2 text-slate-700">{r.category?.name ?? '—'}</td>
