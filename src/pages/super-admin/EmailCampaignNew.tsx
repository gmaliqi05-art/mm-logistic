...
export default function EmailCampaignNew() {
  ...
  return (
    <div className="p-4 lg:p-6">
      ...
      <div className="mb-6 -mx-1 flex items-center gap-1 overflow-x-auto px-1 pb-1 sm:gap-2">
        {steps.map((s, i) => {
          const Icon = s.icon;
          const done = step > s.n;
          ...
          return (
            <div key={s.n} className="flex shrink-0 items-center gap-2">
              ...
              <span className={`hidden whitespace-nowrap text-sm font-medium sm:inline ${active ? "text-slate-900" : "text-slate-500"}`}>{s.label}</span>
              {i < steps.length - 1 && <div className={`h-0.5 w-6 shrink-0 sm:mx-2 sm:w-10 ${done ? "bg-teal-600" : "bg-slate-200"}`} />}
            </div>
          );
        })}
      </div>
