...
export function detectDocumentProjection(canvas: HTMLCanvasElement): ProjectionRect | null {
  ...
  const gxCols = new Float32Array(w);
  const gyRows = new Float32Array(h);
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      const idx = y * w + x;
      const p00 = gray[idx - w - 1], p01 = gray[idx - w], p02 = gray[idx - w + 1];
      const p10 = gray[idx - 1],                        p12 = gray[idx + 1];
      const p20 = gray[idx + w - 1], p21 = gray[idx + w], p22 = gray[idx + w + 1];
  ...
  function smooth(arr: Float32Array, radius: number): Float32Array {
    ...
    for (let i = 0; i < arr.length; i++) {
      let s = 0, n = 0;
      for (let k = Math.max(0, i - radius); k <= Math.min(arr.length - 1, i + radius); k++) {
        s += arr[k]; n++;
      }
      out[i] = n ? s / n : 0;
    }
  ...
  const smX = smooth(gxCols, Math.max(2, Math.round(w / 80)));
  const smY = smooth(gyRows, Math.max(2, Math.round(h / 80)));

  function topPeaks(arr: Float32Array, minSep: number): number[] {
  ...
  const xPeaks = topPeaks(smX, Math.round(w * 0.2));
  const yPeaks = topPeaks(smY, Math.round(h * 0.2));
  if (xPeaks.length < 2 || yPeaks.length < 2) return null;

  xPeaks.sort((a, b) => a - b);
  yPeaks.sort((a, b) => a - b);

  const left = xPeaks[0];
  const right = xPeaks[xPeaks.length - 1];
  const top = yPeaks[0];
  const bottom = yPeaks[yPeaks.length - 1];
  ...
  const innerMean = innerCount ? innerSum / innerCount : meanLum;
  const contrast = Math.abs(innerMean - meanLum);
  const confidence = Math.min(1, (contrast / 40) * ((rw * rh) / (w * h)) * 2);
